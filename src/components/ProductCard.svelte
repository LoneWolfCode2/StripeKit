    ...
    basket.update(n =>
    {
       let i =  basket_array.findIndex(e=> e.id === productID)
        if(i>=0){
            ...
            n.push({name: product.name, id: productID, qty:1, imgURL:product.images[0], price: product.default_price.unit_amount})
        }
       return n
    });
...
>
	...
	<div class="flex justify-between pt-4">
		...
		<div class="w-8 cursor-pointer" on:click={()=>{addToBasket(product.id)}}>
			<FaShoppingBasket  />
		</div>
	</div>
	{#if !product.active}
